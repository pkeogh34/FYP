...
public:
	...
	NNt1(int t_NI, int t_NO, int *t_NK) : NI(t_NI), NO(t_NO) {
		NK = new int[NI];
		NItot = 0;
		for (int i = 0; i < NI; i++) {
			NK[i] = t_NK[i];
	...
	NNt1(int t_NI, int t_NIr, int t_NO, int *t_NK,
		 int t_outp = 1, int t_inp = 1, int t_which = 1) : NI(t_NI), NIr(t_NIr), NO(t_NO), outp(t_outp), inp(t_inp) {
		int i;
		NK = new int[NI];
		NItot = 0;
		for (i = 0; i < NI; i++) {
		...
		which = 2;
		upper = new Layer_tanh(NO, NK, NI, NIr);
